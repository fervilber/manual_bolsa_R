...
En este manual vamos a describir cómo sacar provecho del lenguaje R para aplicaciones de bolsa, análisis de carteras y valores. Trataremos de exponer de manera sencilla los conceptos y poner muchos ejemplos prácticos para extender el uso de R en el área financiera y de bolsa de valores.

No describiremos los usos básicos del lenguaje de programación R, para iniciarse es mejor acudir antes a manuales como el [https://drive.google.com/open?id=1EoLm-rqr5eikmpodb90uIGyju6E1jBjZ](Manual de R para Científicos de datos).

R es un lenguaje muy completo gracias a que miles de colaboradores realizan programaciones intensivas de temas concretos y específicos y las ponen a disposición del resto de gente como paquetes. Las librerías o paquetes añaden funcionalidades a R incrementando la utilidad del lenguaje y del entorno asociado.
En este manual describiremos los principales paquetes para trabajo de datos de bolsa y financieros, algunos son extremadamente potentes y no tienen nada que envidiar a aplicaciones especificas y de pago. El único aspecto que diferencia a R es el aspecto axterior o de usabilidad de las aplicaciones, pero el nucleo al que se tiene acceso de manera gratuita y desinteresada es enorme.

Los amantes de la Bolsa tienen a su disposicion miles de aplicaiones informáticas de análisis, gestión de carteras, representación de gráficos complejos, con estudios indicadores y sistemas preprogramados, entonces... ¿qué puede aportar R?, bueno, la respuesta para mi es sencilla, R aporta todo lo que tienen esas aplicaciones y mucho más, porque lo fundamental de R es que no tiene límites, es que no estás limitado por los indicadores del programa, los sistemas  las estrategias preconcevidas que han programado otros para tí, con R lo puedes hacer tu y lo mejor es que puedes hacer cosas diferentes, cosas, incluso básicas de las que carecen el resto de aplicaciones normales para el gran público.

La desventaja es que requiere un aprendizaje y una familiarización con el programa y su lenguaje, cuanto más se dominine más amplias son las opciones.

La principal ventaja e R es que puedes aplicar técnicas no convencionales, sistemas diferentes que no podrías desarrollar con las aplicaciones (algunas maravillosas y estupendas) comerciales. Además tienes a tu disposición un trasfondo de funiones enorme de base matemática y científica quepuedes aplicar a los datos, con tratamientos estadísticos complejos y graficos personalizados a demanada que no podrás conseguir de otra forma.

## Estructura del manual
Hemos dividido el documento en un capítulo por cada librería. Conocer el uso y lo que podemos obtener de estos paquetes es el objetivo principal, pues nos dará el conocimiento base desde el acceso a los datos hasta la realización de análisis complejos. Nos centraremos en explicar cada paquete indivicualmente para formar el cuerpo base de heramientas, y después aplicaremos esas herremientas en los capitulos de práctica.

Las librerías que forman la caja de herramientas básica para analisis de valores son:
...
Dedicaremos un capitulo a cada una de los paquetes o conjuntos de paquetes enumerados, con ejemplos básicos de las funciones más importantes. El paquete más completo es sin duda `quantstrat`, que forma un cuerpo completo de analisis, pero muchas veces los datos o valores que queremos estudiar no buscamos un analisis profundo ni complejo, sino la vision de datos o representaciones rápidas, para lo que los otros paquetes son perffectamente válidos y más sencillos de comprender y usar. 


# Paquete `quantmod`
`quandtmod` es un paquete fundamental para los que nos gusta la inversión, pues contiene funciones básicas para el trazado y para *descarga de datos* financieros de la web. 
Una de las cosas que más me gusta del mundo de la inversión de bolsa es la facilidad para la obtención de datos, algo que no es fácil en el resto de ramas de cualquier ciencia. En las cuestiones económicas, por tradición de analisi técnico se han puesto a disposición pública multitud de datos y series temporales que proporcionan un delicioso comienzo a los amantes del analisis de datos. `quandmod` nos facilita mucho la obtencion de estos datos haciendolo aún más sencillo.

## Obtener datos de internet

El primer paso para trabajar en inversiones es obtener datos. Por suerte, la librería `quantmod` nos proporciona una función versatil para capturarlos de servicios web: `getSymbols()`. Esta función permite descargar datos, en concreto series temporales de valores de multiples webs públicas.

Por defecto el comando `getSymbols()` devuelve un objeto **con el mismo nombre** que el símbolo (ticker) que se solicita, por lo que no hace falta asignarlo a una variable, pero si se desea asignarlo a una tenemos que establecer la propiedad `auto.assign=FALSE` al hacer la llamada de la función.
